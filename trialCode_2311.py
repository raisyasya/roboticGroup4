...
def portClose():
    portHandler.closePort() ##直到啟動不會開起

def torqueEnable(id, value): ##如果要自己手動轉 關之
...
def servoRead(id):
    ...
    print("[ID:%03d]  PresPos:%03d" % (id, dxl_present_position)) ##根據前面的位置填入後面的數字
...
def wheelMode(id, boolean):
    if boolean == WHEEL_ENABLE: ##兩個都0就是JOINTMOVE
        angleLimit_r =0
        angleLimit_l = 0
    elif boolean == WHEEL_DISABLE : ##反之驗WHEEL MOVE
        angleLimit_r = 0
        angleLimit_l = 1023
...
if __name__ == "__main__":
    ...
    while 1:
        ...
        for i in range(3):
            torqueEnable(DXL_IDs[i], TORQUE_DISABLE)
            wheelMode(DXL_IDs[i], WHEEL_DISABLE)
            servoSpeed(DXL_IDs[i], 1000)
            #servoWrite(DXL_IDs[i], gg)
            gg=gg+aa

            if (gg>upLim or gg<downLim):
                aa = -aa
        #while 1:
            # upLim = upLim - 10*tt
            # downLim = downLim + 10*tt2
